...
void				process_input_sha256(t_ssl_data *input, t_ssl_data *output)
{
	...
	{
		process_block_sha256(input_copy->data + (i * block_size), output->data);
	}
...
void				command_sha256(t_ssl_env *env, char **args)
{
	...
	if (env->flags.p == true || (env->file_args == NULL && env->flags.s == false))
	{
		...
		if (env->flags.p == true)
			display_hash(env, inputstr, "SHA256", output, SSL_STDIN_INPUT);
		else
			display_hash(env, NULL, "SHA256", output, SSL_STDIN_INPUT);
		free(inputstr);
	...
	if (env->flags.s == true)
	{
		...
		process_input_sha256(input, output);
		display_hash(env, env->flags.s_arg, "SHA256", output, SSL_STRING_INPUT);
		data_soft_reset(input);
		data_soft_reset(output);
	}
	...
	if (env->file_args != NULL)
	{
		for (uint32_t i = 0; env->file_args[i] != NULL; i++)
		{
			if (gather_full_input(input, env->file_args[i]) == false)
				continue;
			process_input_sha256(input, output);
			display_hash(env, env->file_args[i], "SHA256", output, SSL_FILE_INPUT);
			data_soft_reset(input);
			data_soft_reset(output);
		}
	}
